...
public class PhpParser {
	...
	private String[] stopword = {"_construct","_destruct","protected","abstract","continue","declare","foreach","function","default","private","public","return","static","switch","(string)","(object)","(float)","(array)","(bool)","(int)","object","string","while","class","const","array","break","echo","elseif","final","print","float","bool","int","xor","<?php","$this","case","else","and","new","for","do","if","or",
								"===","!==","<?","?>","==","!=","<<",">>","++","--","==","!=","<>","<",">",">=","<=","&&","||","*=","/=","%=","+=","-=","&=","|=","^=","<<=",">>=","->",".=","&","|","^","*","/","%","+","-","!","=",";","{","}","(",")","\"","."};  
	
	public PhpParser(String filename){
		
		this.term = new TerminalSymbol();
		//read script from file, parse, and save it to local variable (scriptTokens)
...
//		System.out.println("check number: "+this.isNumber("token"));
	...
	public ArrayList tokenizing(String script){
        ...
        StringBuffer temp = new StringBuffer(); //save the temporary string of single token
        
        /* listing the stopword position in the script, this special stopword will also become a token*/
        for(int x=0; x<stopword.length;x++) {
        	int i=0;
	   		while((i=(script.indexOf(stopword[x],i)+1))>0){
	   			boolean contain=false;
	   	        for(int y=0; y<stopWordPos.size();y++) {
	   	        	if(i>=stopWordPos.get(y)&&i<=(stopWordPos.get(y)+(stopWordStr.get(y).toString().length()))) {
	   	        		contain = true;
	   	        	}
	   	        }
	   	        if(contain==false) {
	   	        	...
	   	        	if((charpivot>=97&&charpivot<=122) || charpivot==95) {
	   	        		if(i==0 || (int)script.charAt(i-1)==32 || (int)script.charAt(i-1)==10 || (int)script.charAt(i-1)==40){
...
//        }
		...
		for(int a=0; a< script.length();a++) {
	        ...
	        if(isquote==false) {
	        	...
	        	//check if character is in index position of stopword, if true, that character is
		        ...
		        else{ //otherwise collect char from another string and make it a token
	        		if(r==32 || r==10 || r==9) { //check of char is space, line feed, or tab
	        			if(!temp.toString().equals("")){
	        				tokens.add(temp.toString());
		        			temp=new StringBuffer();
		        			count++;
	...
	public void program() {
		if(this.token.equalsIgnoreCase("<?php") || this.token.equalsIgnoreCase("<?")) {
			this.token = this.nextToken();
			phpScript();
	...
	public boolean isIdentifier(String token){
		if((int)token.charAt(0)!=95 && !((int)token.charAt(0)>=65 && (int)token.charAt(0)<=90) && !((int)token.charAt(0)>=97 && (int)token.charAt(0)<=122)) {
			return false;
		}
		for(int i=1;i<token.length();i++){
			if((int)token.charAt(i)!=95 && !((int)token.charAt(i)>=65 && (int)token.charAt(i)<=90) && !((int)token.charAt(i)>=97 && (int)token.charAt(i)<=122) && !((int)token.charAt(i)>=48 && (int)token.charAt(i)<=57)){
				return false;
			}
		}
		return true;
	...
	public boolean isString() {
		if((int)this.token.charAt(0)==34 && (int)this.token.charAt(this.token.length()-1)==34){
			this.token=this.nextToken();
			return true;
		}
		else if((int)this.token.charAt(0)==39 && (int)this.token.charAt(this.token.length()-1)==39){
			this.token=this.nextToken();
			return true;
		}
		return false;
	...
	public boolean isDigit() {
		if((int)this.token.charAt(0)>=48 && (int)this.token.charAt(0)<=57) {
			//this.token=this.nextToken(); 
			return true;
		}
		return false;
	...
	public String follow(ArrayList string){
